...
{else}
    <form class="form-alta" action="{$base}edit-product/{$product->id_product}" method="post">
        <input placeholder="Nombre del producto" type="text" value="{$product->name}" name="name" id="name" required>                <textarea placeholder="descripcion" type="text"  name="description" id="description" required>{$product->description}</textarea>
